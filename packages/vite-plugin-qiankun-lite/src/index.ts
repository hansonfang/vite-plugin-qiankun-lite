              ...
              if (typeof htmlStr === "string") {
                ...
                moduleScripts$.each((_, moduleScript) => {
                  ...
                  if (scriptContent.includes(`${config.base}@react-refresh`)) {
                    ...
                    inlineModuleScriptToGeneralScript(
                      moduleScript$,
                      publicPath,
                    );
                  }
 ...
 */
function inlineModuleScriptToGeneralScript(
  script$: Cheerio<Element>,
  publicPath: string,
) {
  ...
  const dynamicImports = imports
    .map(({ names, path }) => {
      // 移除 */ 前缀，并确保路径以 / 开头（但不重复）
      const withoutPrefix = path.replace(/^\*\//, "");
      const normalizedPath = withoutPrefix.startsWith("/")
        ? withoutPrefix
        : `/${withoutPrefix}`;
      if (names) {
